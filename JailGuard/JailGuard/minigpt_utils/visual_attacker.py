...
class PreferenceAttacker:
    ...
    def attack_constrained(self, text_format, img, batch_size=8, num_iter=2000, alpha=1 / 255, epsilon=128 / 255):
        ...
        for _ in tqdm(range(num_iter)):
            ...
            batch_samples = random.sample(self.targets, batch_size)
            batch_prompts = [_['prompt'] for _ in batch_samples]
            batch_targets = [_['response'] for _ in batch_samples]

            text_formats = [text_format] * batch_size

            x_adv = x + adv_noise
            x_adv = normalize(x_adv)

            prompt = prompt_wrapper.PreferencePrompt(
                model=self.model, text_formats=text_formats, text_prompts=batch_prompts, img_prompts=[[x_adv]]
            )
            prompt.img_embs = prompt.img_embs * batch_size
            prompt.update_context_embs()
