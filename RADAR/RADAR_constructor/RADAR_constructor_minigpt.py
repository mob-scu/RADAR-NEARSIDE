...
judger_client = OpenAI(api_key="your openai key here")
def parse_args():

    parser = argparse.ArgumentParser(description="Demo")

    parser.add_argument("--image_fold", type=str,
                        help="Image file")
    parser.add_argument("--origin_fold", type=str,
                        help="Image file")
...
def request(img, img_origin, query, url):
    while True:
        try:
            url = url
            input_data = {
            ...
            results_origin = requests.post(url, timeout=300, json=input_original_data)
            result_origin = json.loads(results_origin.text)['outputs'][0]['data'][0]
    ...
    # 返回prompt和harmful对比结果
    return query, result, result_origin
...
if __name__ == '__main__':
    ...
    for img in tqdm(imgs):
        ...
        for query in tqdm(querys):
            if cnt >= 30:
                break
            q, a, ao = request(imgbase64, originbase64, query, args.url)
            if (predict([q], [a])[0] == True and predict([q], [ao])[0] == False):
